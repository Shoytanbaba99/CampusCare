    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="bg-[#111827] border border-[#1F2937] rounded-xl overflow-hidden shadow-xl">
        <div className="px-6 py-4 border-b border-[#1F2937] flex items-center justify-between">
          <h2 className="font-semibold text-base text-[#F9FAFB]">Audit Event History</h2>
        ...
        {!auditLogs || auditLogs.length === 0 ? (
          ...
          <div className="divide-y divide-[#1F2937]">
            {auditLogs.map((log) => {
              ...
              return (
                <div key={log.id} className="p-5 space-y-3 hover:bg-[#1F2937]/40 transition-all">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                    <div className="flex items-center gap-2">
                      <span className="font-mono font-bold text-xs px-2 py-0.5 rounded bg-[#6366F1]/10 text-[#6366F1] border border-[#6366F1]/20">
                        {log.action}
                      </span>
                      <span className="text-[#9CA3AF] flex items-center gap-1">
                        <User className="w-3.5 h-3.5 text-[#6366F1]" />
                        Actor: <strong className="text-[#F9FAFB]">{actorName}</strong> ({actorRole.toUpperCase()})
                      </span>
                    </div>
                  ...
                  {(log.old_state || log.new_state) && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[11px] pt-1">
                      {log.old_state && (
                        <div className="p-3 bg-[#090D16] rounded-lg border border-[#1F2937] space-y-1">
                          <span className="text-[#9CA3AF] font-semibold uppercase flex items-center gap-1">
                            <FileCode className="w-3 h-3 text-red-400" />
                            Previous State (JSON)
                          </span>
                          <pre className="text-red-400/90 font-mono overflow-x-auto whitespace-pre-wrap">
                      ...
                      {log.new_state && (
                        <div className="p-3 bg-[#090D16] rounded-lg border border-[#1F2937] space-y-1">
                          ...
                          <pre className="text-emerald-400/90 font-mono overflow-x-auto whitespace-pre-wrap">
